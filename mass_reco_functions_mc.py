...
def compute_jet_pair_properties_all_events(events, j_matched, q_matched, prefix="max_weight",
                                            btag_field="btagUParTAK4B"):
    """
    btag_field: name of the b-tag discriminator branch on JetGood to read for
    the "*_jet_btag_score*" outputs below. Defaults to "btagUParTAK4B" for
    backward compatibility, but callers should pass the year-resolved field
    name (e.g. self._btag_field in workflow_mc_7.py, driven by
    params.btagging.working_point.<year>.btagging_algorithm) -- 2022/2023
    samples use "btagRobustParTAK4B", 2018 would use "btagDeepFlavB", etc.
    """
    ...
    for event_idx in range(t):
        ...
        if first_match == 1. and second_match == 0.:
            ...
            if original_hj1 < n_jets_good and original_hj2 < n_jets_good:
                ...
                massreco_chosen_pair_correct_jet_btag_score = events["JetGood"][btag_field][event_idx][original_hj1]
                massreco_chosen_pair_wrong_jet_btag_score   = events["JetGood"][btag_field][event_idx][original_hj2]
        ...
        # ----- case: second jet matched, first wrong -----
            ...
            if original_hj1 < n_jets_good and original_hj2 < n_jets_good:
                ...
                massreco_chosen_pair_correct_jet_btag_score = events["JetGood"][btag_field][event_idx][original_hj2]
                massreco_chosen_pair_wrong_jet_btag_score   = events["JetGood"][btag_field][event_idx][original_hj1]
        ...
        if first_match == 0. and second_match == 0.:
            ...
            if original_hj1 < n_jets_good and original_hj2 < n_jets_good:
                ...
                massreco_chosen_pair_wrong_jet_btag_score        = events["JetGood"][btag_field][event_idx][original_hj1]
                massreco_chosen_pair_wrong_jet_btag_score_second = events["JetGood"][btag_field][event_idx][original_hj2]
        ...
        if first_match == 1. and second_match == 1.:
            ...
            massreco_chosen_pair_correct_jet_btag_score        = events["JetGood"][btag_field][event_idx][original_hj1]
            massreco_chosen_pair_correct_jet_btag_score_second = events["JetGood"][btag_field][event_idx][original_hj2]
...
def compute_jet_pair_properties_all_events_4(events, j_matched, q_matched,
                                              btag_field="btagUParTAK4B"):
    """
    btag_field: name of the b-tag discriminator branch on JetGood used for the
    top-4-by-btag check (pair_in_the_4leadingbtagscore / mass_reco_had_a_solution
    below). Defaults to "btagUParTAK4B" for backward compatibility -- callers
    should pass the year-resolved field name (e.g. self._btag_field in
    workflow_mc_7.py, driven by params.btagging.working_point.<year>.btagging_algorithm).
    ...
    for event_idx in range(len(events)):
        ...
        btag_scores = ak.to_numpy(events["JetGood"][btag_field][event_idx])

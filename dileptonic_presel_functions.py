...
############## dileptonic preselections, both standard and SPANet conditions applied ###########
...
def dileptonic_MaskNegTagVal(events, params, year, processor_params, sample, isMC, **kwargs):
    ...
    mask = (
        ...
        & mask_jetVetoMap
        & channel_cut
        & ak.all(events.JetGood.btagUParTAK4B >= 0, axis=1)  # <-- Exclude events with any jet tag value < 0
    )
    return ak.where(ak.is_none(mask), False, mask)
